...
router.get('/', (req, res) => {
    const { LINKEDIN_CLIENT_ID ,SERVER_URI  } = process.env;

    if (!LINKEDIN_CLIENT_ID || !SERVER_URI) {
        return res.status(500).send('Missing LinkedIn environment variables.');
    }

    const encodedRedirect = encodeURIComponent(`${SERVER_URI}/api/linkedin/callback`);
